    ...
    try:
        ...
        for idx, row in fight_data.iterrows():
            ...
            for fighter_num in ["Fighter1", "Fighter2"]:
                ...
                if stats_before["NumFights"] > 0:
                    ...
                    if stats_before["LastFightDate"] is not None:
                        days_since_last_fight = (
                            event_date - stats_before["LastFightDate"]
                        ).days
                        fight_data.at[idx, f"{fighter_num}_TimeSinceLastFight"] = (
                            days_since_last_fight
                        )
                    else:
                    ...
                    fight_data.at[idx, f"{fighter_num}_AvgSubmissionAttempts"] = (
                        stats_before["TotalSubmissionAttempts"]
                ...
                fight_data.at[idx, f"{fighter_num}_NoContests"] = stats_before[
                    "NoContests"
                ]
            ...
            for fighter_num in ["Fighter1", "Fighter2"]:
                ...
                stats["TotalControlTime"] += (
                    control_time if not np.isnan(control_time) else 0
                )

                sub_attempts = float(row[f"{fighter_num}_Submission_Attempts"])
                stats["TotalSubmissionAttempts"] += (
                    sub_attempts if not np.isnan(sub_attempts) else 0
                )

                leg_strikes = row[f"{fighter_num}_Leg_Strikes_Landed"]
                stats["TotalLegStrikes"] += (
                    leg_strikes if not np.isnan(leg_strikes) else 0
                )

                clinch_strikes = row[f"{fighter_num}_Clinch_Strikes_Landed"]
                stats["TotalClinchStrikes"] += (
                    clinch_strikes if not np.isnan(clinch_strikes) else 0
                )
                ...
                stats["TotalStrikesLanded"] += (
                    strikes_landed if not np.isnan(strikes_landed) else 0
                )
                ...
                stats["TotalTakedownsLanded"] += (
                    takedowns_landed if not np.isnan(takedowns_landed) else 0
                )
                stats["TotalTakedownsAttempted"] += (
                    takedowns_attempted if not np.isnan(takedowns_attempted) else 0
                )
